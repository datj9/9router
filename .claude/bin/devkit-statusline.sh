...
# ── Auth status (from cached auth-check hook result) ──────────────
AUTH_CACHE="/tmp/devkit-auth-status.json"
AUTH_EXPIRED=""
if [ -f "$AUTH_CACHE" ]; then
  AUTH_LOGGED_IN="$(jq -r '.loggedIn // false' "$AUTH_CACHE" 2>/dev/null || echo 'false')"
...
# Auth expired indicator — red blinking warning when session is not logged in
AUTH_PART=""
if [ -n "$AUTH_EXPIRED" ]; then
  AUTH_PART=" $(fg256bold 196)SESSION EXPIRED${RESET}"
fi

LABEL="$(fg256dim 245)"  # muted gray for ctx/sess/week labels
printf '%s%s%s%s %s %s %s %sctx%s %s %s %s %ssess%s %s %s %s %sweek%s %s %s' \
  "$SPINNER_PART" \
  "$PROJECT_PART" \
  "$BRANCH_PART" \
  "$AUTH_PART" \
